...
const REPO = path.join(SENA, '..');            // the mulesoo repo — brand assets live here
...
const FONT_DIR = path.join(REPO, 'assets', 'fonts');
const FONTS = [
...
const CREDIT = dataUri(path.join(REPO, 'public', 'brand', 'mulesoo-credit-compact-on-dark.png'));
...
const JSBARCODE = fs.readFileSync(
  path.join(REPO, 'node_modules', 'jsbarcode', 'dist', 'JsBarcode.all.min.js'),
  'utf8'
);
...
(async () => {
  ...
  try {
    ...
    const jsQR = require(path.join(REPO, 'node_modules', 'jsqr')).default
      || require(path.join(REPO, 'node_modules', 'jsqr'));
