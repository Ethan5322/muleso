...
do $$ begin
  create type guest_id_status as enum ('active', 'used', 'expired');
exception when duplicate_object then null; end $$;
...
-- ── hotels ──────────────────────────────────────────────────────────────────
-- Not in §6, added deliberately: this system is sold to many hotels. Every
-- other table hangs off this one so a second client is a row, not a database.
create table if not exists hotels (
  id                  uuid primary key default gen_random_uuid(),
  name                text not null,
  phone               text not null,                    -- the published number guests dial
  email               text,
  address             text,
...
-- bookings. Modelling one row per physical room would force Sena to pick a
...
create index if not exists rooms_hotel_idx on rooms(hotel_id) where is_active;
...
create index if not exists calls_hotel_started_idx on calls(hotel_id, started_at desc);
...
create table if not exists guests (
  ...
  email       text,
  nationality text,
  notes       text,                                      -- preferences, for returning guests (§1)
  created_at  timestamptz not null default now()
);
create index if not exists guests_hotel_phone_idx on guests(hotel_id, phone);
...
create table if not exists bookings (
  ...
  -- hotel silently loses sellable nights to abandoned calls.
...
create table if not exists guest_ids (
  ...
  status              guest_id_status not null default 'active',
  used_at             timestamptz,
...
-- expired bookings free the room. Date overlap is half-open [check_in,
-- check_out) — a guest leaving on the 5th does not clash with one arriving on
-- the 5th, which is how hotels actually work.
-- ============================================================================
create or replace function rooms_taken(
  p_room_id  uuid,
  p_check_in date,
  p_check_out date
) returns int
...
as $$
  select
    ...
    r.capacity,
    r.amenities,
    (r.inventory - rooms_taken(r.id, p_check_in, p_check_out)) as free_units,
  ...
  where r.hotel_id = p_hotel_id
    ...
    and (r.inventory - rooms_taken(r.id, p_check_in, p_check_out)) > 0
  order by r.rate_cents asc;
$$;

-- ── hold_room: the anti-double-booking gate ─────────────────────────────────
...
create or replace function hold_room(
  p_hotel_id     uuid,
  p_room_id      uuid,
  p_check_in     date,
  p_check_out    date,
...
begin
  ...
  select * into v_room from rooms where id = p_room_id and hotel_id = p_hotel_id for update;
  if not found then raise exception 'unknown room % for hotel %', p_room_id, p_hotel_id; end if;

  v_taken := rooms_taken(p_room_id, p_check_in, p_check_out);
  ...
  insert into bookings (hotel_id, room_id, call_id, reference, check_in, check_out,
...
-- ── expire_stale_holds: run on a schedule (n8n, every 5 min) ────────────────
...
-- honest about what actually happened on those calls.
...
-- ── knock_out_guest_id: single-use check-in (§7 ID lifecycle rule) ──────────
...
-- not both succeed.
create or replace function knock_out_guest_id(
...
begin
  ...
  if not found then
    ...
    select * into v_id from guest_ids where verification_number = p_verification_number;
    if not found then
  ...
  select * into v_guest from guests where id = v_booking.guest_id;
