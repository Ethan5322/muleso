...
delete from hotels where is_demo;
...
insert into rooms (hotel_id, name, description, plan, rate_cents, capacity, inventory, amenities)
...
declare
  ...
  v_free  int;
begin
  select id into v_hotel from hotels where is_demo;
  if v_hotel is null then raise exception 'demo hotel did not seed'; end if;

  select count(*) into v_rooms from rooms where hotel_id = v_hotel;
  ...
  select count(*) into v_free
    from check_availability(v_hotel, current_date + 7, current_date + 9, 2);
  if v_free = 0 then raise exception 'no availability returned for a 2-night stay'; end if;

  raise notice 'Demo hotel seeded: % room types, % sellable for 2 guests next week.', v_rooms, v_free;
end $$;
