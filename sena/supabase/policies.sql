-- ============================================================================
-- Sena — Row Level Security (CLAUDE.md §9)
--
-- Threat model, stated plainly: this database holds guests' full names, phone
-- numbers, email addresses and nationalities. Under POPIA that is personal
-- information, and a leak is the hotel's liability and MuleSoo's reputation.
--
-- Two principles:
--   1. The public (the `anon` key that ships in any browser bundle) gets
--      NOTHING. Not a room list, not a booking, nothing. Sena talks to the
--      database through the service_role key from the server side only.
--   2. A logged-in staff member sees their OWN hotel and no other. Hotel A's
--      manager must never be one forged request away from hotel B's guests.
--
-- Run after schema.sql.
-- ============================================================================

-- Who works at which hotel. A staff row is created by the owner (or by MuleSoo
-- on setup); auth.uid() comes from Supabase Auth.
create table if not exists hotel_staff (
  user_id    uuid not null references auth.users(id) on delete cascade,
  hotel_id   uuid not null references hotels(id) on delete cascade,
  role       text not null default 'reception' check (role in ('owner', 'manager', 'reception')),
  created_at timestamptz not null default now(),
  primary key (user_id, hotel_id)
);

alter table hotels             enable row level security;
alter table rooms              enable row level security;
alter table calls              enable row level security;
alter table guests             enable row level security;
alter table bookings           enable row level security;
alter table guest_ids          enable row level security;
alter table payments           enable row level security;
alter table notifications_log  enable row level security;
alter table hotel_staff        enable row level security;

-- Force RLS even for the table owner, so a mistake in a migration script or a
-- future `postgres`-role query cannot quietly bypass the whole model.
alter table guests   force row level security;
alter table bookings force row level security;
alter table payments force row level security;

-- ── The tenancy predicate ───────────────────────────────────────────────────
-- SECURITY DEFINER so the policy can read hotel_staff without recursing back
-- into hotel_staff's own RLS policy (a classic infinite-recursion footgun).
create or replace function is_staff_of(p_hotel_id uuid)
returns boolean
language sql
stable
security definer
set search_path = public
as $$
  select exists (
    select 1 from hotel_staff s
    where s.hotel_id = p_hotel_id
      and s.user_id = auth.uid()
  );
$$;

-- ── Policies ────────────────────────────────────────────────────────────────
-- No policy is written for `anon`. In Postgres RLS, absence of a policy means
-- deny — so the public key already gets nothing. That is deliberate, not an
-- omission.
--
-- Staff get SELECT on their hotel. They do NOT get blanket INSERT/UPDATE:
-- bookings are created by Sena through the service_role key, which bypasses RLS
-- entirely. The one thing reception must be able to do by hand is check a guest
-- in, and that goes through knock_out_guest_id(), not a raw UPDATE.

create policy staff_read_hotel on hotels
  for select to authenticated using (is_staff_of(id));

create policy staff_read_rooms on rooms
  for select to authenticated using (is_staff_of(hotel_id));

create policy staff_read_calls on calls
  for select to authenticated using (is_staff_of(hotel_id));

create policy staff_read_guests on guests
  for select to authenticated using (is_staff_of(hotel_id));

create policy staff_read_bookings on bookings
  for select to authenticated using (is_staff_of(hotel_id));

create policy staff_read_guest_ids on guest_ids
  for select to authenticated using (
    exists (select 1 from bookings b where b.id = guest_ids.booking_id and is_staff_of(b.hotel_id))
  );

create policy staff_read_payments on payments
  for select to authenticated using (
    exists (select 1 from bookings b where b.id = payments.booking_id and is_staff_of(b.hotel_id))
  );

create policy staff_read_notifications on notifications_log
  for select to authenticated using (
    booking_id is null
    or exists (select 1 from bookings b where b.id = notifications_log.booking_id and is_staff_of(b.hotel_id))
  );

create policy staff_read_own_staff_rows on hotel_staff
  for select to authenticated using (user_id = auth.uid());

-- Owners and managers may correct room rates and inventory from the dashboard.
create policy managers_write_rooms on rooms
  for update to authenticated
  using (
    exists (select 1 from hotel_staff s
             where s.hotel_id = rooms.hotel_id
               and s.user_id = auth.uid()
               and s.role in ('owner', 'manager'))
  )
  with check (
    exists (select 1 from hotel_staff s
             where s.hotel_id = rooms.hotel_id
               and s.user_id = auth.uid()
               and s.role in ('owner', 'manager'))
  );

-- ── Front-desk check-in ─────────────────────────────────────────────────────
-- knock_out_guest_id() flips a booking to checked_in and burns the QR. Reception
-- has no UPDATE grant on bookings, so this SECURITY DEFINER wrapper is the only
-- door — and it can only ever do that one thing.
create or replace function staff_check_in(
  p_verification_number text
) returns table (ok boolean, reason text, booking_reference text, guest_name text)
language plpgsql
security definer
set search_path = public
as $$
declare
  v_hotel_id uuid;
begin
  select b.hotel_id into v_hotel_id
    from guest_ids g
    join bookings b on b.id = g.booking_id
   where g.verification_number = p_verification_number;

  if v_hotel_id is null then
    return query select false, 'unknown code', null::text, null::text;
    return;
  end if;

  -- The scanner must work at the hotel the code belongs to.
  if not is_staff_of(v_hotel_id) then
    return query select false, 'not authorised for this property', null::text, null::text;
    return;
  end if;

  return query select * from knock_out_guest_id(p_verification_number, auth.uid()::text);
end;
$$;

revoke all on function staff_check_in(text) from public, anon;
grant execute on function staff_check_in(text) to authenticated;

-- Availability is read by Sena (service_role). Nothing here is granted to anon:
-- a public rate-scraper is not a feature the hotel asked for.
revoke all on function check_availability(uuid, date, date, int) from public, anon;
revoke all on function hold_room(uuid, uuid, date, date, int, uuid)  from public, anon;
revoke all on function knock_out_guest_id(text, text)                from public, anon;
revoke all on function expire_stale_holds()                          from public, anon;
