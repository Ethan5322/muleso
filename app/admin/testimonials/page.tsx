        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {items.map((t) => (
            <div key={t.id} className="bg-[#0A0E17] border border-[#1E3A5F] rounded-xl p-5">
              <div className="flex items-center gap-1 mb-2 text-[#7FB3FF]">
                {Array.from({ length: t.rating || 5 }).map((_, i) => <Star key={i} size={14} className="fill-[#7FB3FF]" />)}
              </div>
          ...
          <div className="bg-[#0A0E17] border border-[#1E3A5F] rounded-2xl p-6 w-full max-w-lg">
            ...
            <div className="space-y-4">
              <textarea
                ...
                rows={3}
                className="w-full bg-[#1A2332] border border-[#1E3A5F] text-white px-4 py-2.5 rounded-lg focus:outline-none focus:border-[var(--color-action-on-dark)] resize-none"
              />
              <div className="grid grid-cols-2 gap-3">
                <input value={editing.author} onChange={(e) => setEditing({ ...editing, author: e.target.value })} placeholder="Author name *" className="bg-[#1A2332] border border-[#1E3A5F] text-white px-4 py-2.5 rounded-lg focus:outline-none focus:border-[var(--color-action-on-dark)]" />
                <input value={editing.role} onChange={(e) => setEditing({ ...editing, role: e.target.value })} placeholder="Role (e.g. CEO)" className="bg-[#1A2332] border border-[#1E3A5F] text-white px-4 py-2.5 rounded-lg focus:outline-none focus:border-[var(--color-action-on-dark)]" />
                <input value={editing.company} onChange={(e) => setEditing({ ...editing, company: e.target.value })} placeholder="Company" className="bg-[#1A2332] border border-[#1E3A5F] text-white px-4 py-2.5 rounded-lg focus:outline-none focus:border-[var(--color-action-on-dark)]" />
                <select value={editing.rating} onChange={(e) => setEditing({ ...editing, rating: Number(e.target.value) })} aria-label="Rating" className="bg-[#1A2332] border border-[#1E3A5F] text-white px-4 py-2.5 rounded-lg focus:outline-none focus:border-[var(--color-action-on-dark)]">
