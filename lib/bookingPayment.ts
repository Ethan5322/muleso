...
export async function sendDepositPaymentEmail(opts: {
  ...
  /** The client's own self-reported budget band, e.g. 'R5,000 - R9,900' or
   *  'Not sure yet'. When it's 'Not sure yet', we must not hand them a firm
   *  50%-of-list-price figure and a live payment button for an amount they
   *  never agreed to — see sendUnsureBudgetEmail below. */
  budget?: string;
}): Promise<{ sent: boolean; reason?: string }> {
  if (!process.env.RESEND_API_KEY) return { sent: false, reason: 'RESEND_API_KEY not set' };

  const budgetUnsure = (opts.budget || '').trim().toLowerCase() === 'not sure yet';
  if (budgetUnsure) {
    return sendUnsureBudgetEmail(opts);
  }
...
/**
 * The deposit email for a client who told the widget they don't know their
 * budget yet ('Not sure yet'). `quoteForService` would still hand back a
 * confident 50%-of-list-price figure — that number is real (it's the site's
 * own published price), but presenting it as a fait accompli, with a live
 * "pay now" button, to someone who explicitly said they hadn't settled on a
 * budget skips the conversation that figure deserves. No Paystack transaction
 * is initialized here; no deposit amount is charged or implied. The client
 * hears from the team first, and gets a real payment link once an amount is
 * actually agreed.
 */
async function sendUnsureBudgetEmail(opts: {
  email: string;
  fullName: string;
  service: string;
  verificationCode: string;
  ...
  const emailRes = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'chatbot@mulesoo.com',
      to: opts.email,
      subject: `Booking fee received — let's agree your deposit together (ref ${opts.verificationCode})`,
      html: `
        <div style="background:#F4F6FB;padding:32px 12px;font-family:'Segoe UI',Helvetica,Arial,sans-serif;">
          <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:560px;margin:0 auto;background:#FFFFFF;border-radius:14px;overflow:hidden;border:1px solid #E4E9F2;">
            <tr>
            ...
            <tr>
              <td style="padding:32px 32px 8px;">
                <p style="margin:0 0 14px;font-size:17px;color:#0A0F1E;">Hi ${escapeHtml(opts.fullName)},</p>
                <p style="margin:0 0 18px;font-size:15px;line-height:1.65;color:#3C4557;">
                  Thanks — your R${BOOKING_FEE_ZAR} booking fee is confirmed and your slot for
                  <strong>${escapeHtml(opts.service)}</strong> is held under the reference below.
                </p>
                <p style="margin:0 0 22px;font-size:15px;line-height:1.65;color:#3C4557;">
                  You let us know you're not sure of your budget yet — so rather than assume a figure,
                  our team will reach out personally to talk through your project and agree on a fair
                  deposit together. Once that's settled, we'll send you a secure payment link for that
                  exact amount, nothing charged or assumed before then.
                </p>
              </td>
            </tr>
            ...
            <tr>
              <td style="padding:0 32px;">
                <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border:1px solid #E4E9F2;border-radius:10px;">
                  ...
                  <tr>
                    <td style="padding:14px 18px;font-size:13px;color:#6B7488;">Reference</td>
                    <td style="padding:14px 18px;font-size:14px;color:#0A0F1E;font-weight:600;text-align:right;letter-spacing:0.5px;">${escapeHtml(opts.verificationCode)}</td>
                  </tr>
                </table>
              </td>
            </tr>
            ...
            <tr>
              <td style="padding:24px 32px 32px;">
                <p style="margin:0;font-size:13px;line-height:1.7;color:#3C4557;">
                  Expect to hear from us within 2 business hours. Any questions in the meantime,
