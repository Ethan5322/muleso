...
export const generateCleanBookingPDF = async (bookingData: BookingData): Promise<void> => {
  try {
    ...
    try {
      ...
      console.warn('⚠️ QR Code generation skipped:', err);
    }
    ...
    const noticeText = [
      ...
      '3. Send signed document via WhatsApp: +27 759 440 377',
    ...
    doc.text('📱 +27 759 440 377', margin + 50, footerY);
